<aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-gradient-to-b from-blue-800 to-blue-950 text-white shadow-xl transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out flex flex-col">
    ...
    <div class="mt-auto p-3 lg:p-4 border-t border-blue-700 bg-blue-900 relative">
        ...
        <div id="profileMenu" class="hidden absolute left-3 right-3 lg:left-4 lg:right-4 bottom-full mb-2 bg-blue-900 rounded-lg border border-blue-700 overflow-hidden shadow-xl z-50">
            <form action="{{ route('logout') }}" method="POST" class="logout-form-feature">
                @csrf
                <button type="submit"
...
<div id="logoutModalFeature" class="fixed inset-0 hidden items-center justify-center bg-black/40 backdrop-blur-sm z-[9999]">
    <div class="bg-white rounded-xl shadow-lg w-[90%] max-w-sm p-5 text-center">
        ...
        <div class="mt-5 flex items-center justify-center gap-2">
            <button type="button" id="cancelLogoutFeature" class="px-4 py-2 rounded-lg bg-gray-200 hover:bg-gray-300 text-gray-700 text-sm font-medium">Batal</button>
            <button type="button" id="confirmLogoutFeature" class="px-4 py-2 rounded-lg bg-red-600 hover:bg-red-700 text-white text-sm font-medium">Logout</button>
        </div>
    </div>
</div>
...
<script>
    (function() {
        ...
        const logoutModal = document.getElementById('logoutModalFeature');
        const cancelLogout = document.getElementById('cancelLogoutFeature');
        const confirmLogout = document.getElementById('confirmLogoutFeature');
        const logoutForms = document.querySelectorAll('.logout-form-feature');
